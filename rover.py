...
class rover(object):
    ...
    I know setters and getters are not required in Python,
    but I thought it would be interesting to show that I know to work with
    the operator library too.
    ...
    def x(self, x):
        ...
        Mars surface)
        """
        if (int(x) < 0 or int(x) > int(self.Mars.x)):
...
It should be value should be < 0 and > {}""".format(str(self.Mars.x)))
    ...
    def y(self, y):
        ...
        Mars surface).
        """
        if (int(y) < 0 or int(y) > int(self.Mars.y)):
    ...
    def forward(self):
        ...
        for spaces in self.Mars.occupied:
            if self.get_current_position() == (spaces[0], spaces[1]):
                raise RuntimeError("I've hit a position that is already taken.\n\
